module MaterialsHelper
  MATERIALS_INFO = "In the context the ExPaNDS/PaNOSC Training Catalogue, a training material is a link to a single online training material sourced by a content provider (such as a text on a Web page, presentation, video, etc.) along with description and other meta information (e.g. ontological categorization, keywords, etc.).\n\n"\
  "The ExPaNDS/PaNOSC Training Catalogue harvests materials automatically from our E-learning platform, including descriptions and other relevant meta-data made available by providers. Materials can also be registered manually.\n\n"\
  "If your website contains training materials that you wish to include in our ExPaNDS/PaNOSC Training Catalogue, please contact the ExPaNDS team (<a href='#{TeSS::Config.contact_email}'>#{TeSS::Config.contact_email}</a>) for further details.".freeze

  ELEARNING_MATERIALS_INFO = "e-Learning materials are curated materials focused on e-Learning.\n\n"\
  "If your website contains e-Learning materials that you wish to include in our ExPaNDS/PaNOSC Training Catalogue, please contact the ExPaNDS team (<a href='#{TeSS::Config.contact_email}'>#{TeSS::Config.contact_email}</a>) for further details.".freeze

  TOPICS_INFO = "The ExPaNDS/PaNOSC Training Catalogue generates a scientific topic suggestion for each resource registered. It does this by
  passing the description and title of the resource to the Bioportal Annotator Web service.
The Annotator Web service finds EDAM terms that match terms in the text. You can then accept or reject these terms in our ExPaNDS/PaNOSC Training Catalogue.
...
Accepting will add a topic to the resource and rejecting will remove the suggestion permanently"
  ...
  def scientific_topic_names_for_autocomplete
    ...
    BLOB::Ontology.instance.all_topics.map(&:preferred_label)
  end
